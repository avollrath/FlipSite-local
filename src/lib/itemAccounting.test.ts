...
describe('item accounting helpers', () => {
  ...
  it('preserves current raw loss behavior for unsold standalone items', () => {
    ...
    expect(getUnsoldResaleAggregateItems([unsold])).toEqual([unsold])
  })
  ...
  it('excludes keepers from resale accounting by status or category', () => {
    ...
    expect(getKeepingAggregateItems([statusKeeper, categoryKeeper])).toEqual([
      statusKeeper,
      categoryKeeper,
    ])
  })
  ...
  it('preserves current bundle child profit and ROI ambiguity', () => {
    const parent = item({
      buy_price: 100,
      is_bundle_parent: true,
      name: 'Bundle',
    ...
    expect(calculateItemProfit(child, [parent, child])).toBe(70)
    expect(calculateItemROI(child, [parent, child])).toBeCloseTo(133.333, 3)
